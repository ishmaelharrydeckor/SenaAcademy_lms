...
async function sendSMS(recipientPhone, messageText) {
  try {
    const response = await fetch('https://sms.arkesel.com/api/v2/sms/send', {
      ...
      signal: AbortSignal.timeout(30000) // 30s timeout for Arkesel server congestion
    });
    const data = await response.json();
...
async function main() {
  ...
  const progressFile = path.join(__dirname, 'kickoff_lock_progress.json');
  ...
  console.log('=============================================================');
  console.log('       SENA ACADEMY CAMPAIGN: SATURDAY ROSTER KICKOFF LOCK');
  console.log('=============================================================');
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const smsText = `Sena Academy: Roster lock is at 6:00 PM today. Live kickoff class begins at 7:00 PM tonight. This is your final chance to complete enrollment at the GHS 100 early-bird rate before it increases to GHS 200 at 6 PM. Secure your seat now: https://senaacademy.org/enroll`;

    const emailSubject = 'Final Call: Founding Builders Roster Lock (6:00 PM Today)';
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
        <h2 style="color: #0f172a; margin-top: 0; font-size: 20px; font-weight: 700; border-bottom: 1px solid #0f172a; padding-bottom: 15px;">🔒 Final Call: Roster Lock at 6:00 PM Today</h2>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">Hi ${firstName},</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">Our live kickoff learning session begins at exactly <strong>7:00 PM GMT tonight</strong> (Saturday, August 1st).</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">To complete database registrations and prepare classroom setup, the cohort roster will lock at <strong>exactly 6:00 PM GMT today</strong>.</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">This is your absolute final opportunity to complete your enrollment at the GHS 100 early-bird rate. After 6:00 PM, all registration links will revert to the standard price of <strong>GHS 200</strong> permanently.</p>
        
        <div style="background-color: #f8fafc; border-left: 4px solid #0f172a; padding: 15px; margin: 25px 0; border-radius: 4px;">
          <h4 style="margin: 0 0 10px 0; color: #0f172a; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Founding Builders Kickoff Schedule:</h4>
          <ul style="margin: 0; padding-left: 20px; font-size: 13.5px; line-height: 1.6; color: #475569;">
            ...
            <li style="margin-bottom: 0;"><strong>Onboarding:</strong> Welcome email with portal credentials and WhatsApp group invites will be delivered immediately upon successful enrollment.</li>
          </ul>
        </div>

        <div style="text-align: center; margin: 30px 0;">
          <a href="https://senaacademy.org/enroll" style="background-color: #0f172a; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 14px; display: inline-block;">Secure My Seat & Enroll for GHS 100</a>
        </div>
    ...
    console.log(`[${i + 1}/${pending.length}] Dispatching kickoff lock alert to ${contact.name}...`);
  ...
  console.log('\nKickoff lock campaign complete.');
}
