...
async function broadcast() {
  console.log('--- STARTING WAITLIST EMAIL BROADCAST ---\n');

  // 1. Fetch all paid emails from payments
  ...
  console.log(`Found ${paidEmails.size} paid Cohort 1 emails to exclude.`);

  // 2. Fetch all waitlist signups
  ...
  if (waitlistErr) {
    console.error('Failed to fetch waitlist:', waitlistErr.message);
    return;
  }

  console.log(`Found ${waitlist.length} raw waitlist entries.`);

  // 3. Deduplicate and filter out paid members & test emails
  ...
  for (const entry of waitlist) {
    ...
    if (paidEmails.has(cleanEmail)) {
      // Excluded paid member
      continue;
    }
  ...
  console.log(`\n🎯 Filtered Target Audience: ${targets.length} unique unpaid waitlist members.\n`);
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      ...
      failCount++;
      console.log(`\nError sending to ${t.email}:`, err.message);
    }
