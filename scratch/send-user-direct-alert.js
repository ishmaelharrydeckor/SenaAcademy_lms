...
if (!brevoApiKey) {
  console.error('[Error] Missing BREVO_API_KEY in .env.local');
  process.exit(1);
}
...
async function main() {
  ...
  for (const user of userEmails) {
    ...
    console.log(`Sending copy to ${user.name} (${user.email})...`);
    const res = await sendEmail(user.email, user.name, emailSubject, emailHtml);
    if (res.success) {
      console.log(`[+] Sent successfully to ${user.email}`);
    } else {
      console.error(`[-] Failed sending to ${user.email}: ${res.error}`);
    }
  }
}
