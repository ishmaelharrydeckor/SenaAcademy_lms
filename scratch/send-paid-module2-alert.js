...
async function main() {
  ...
  console.log('\nStarting Module 2 live session email dispatch in 3 seconds...');
  await new Promise(r => setTimeout(r, 3000));

  for (let i = 0; i < paidStudents.length; i++) {
    const student = paidStudents[i];

    const emailSubject = '🔴 Starting Soon: Module 2 Session (7:00 PM GMT)';
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
        <h2 style="color: #ef4444; margin-top: 0; font-size: 20px; font-weight: 700; border-bottom: 1px solid #ef4444; padding-bottom: 15px;">🔴 Starting Soon: Module 2 Session</h2>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">Hi ${student.name},</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">Our live session for **Module 2** of the Founding Builders Cohort is starting in just 20 minutes (at 7:00 PM GMT tonight)!</p>
    ...
    console.log(`[${i + 1}/${paidStudents.length}] Emailing paid student ${student.name} (${student.email})...`);
    ...
    if (emailRes.success) {
      ...
      console.error(`[-] Email failed. Error: ${emailRes.error}`);
    }

    await new Promise(r => setTimeout(r, 200));
  }

  console.log('\nModule 2 kickoff email dispatch complete.');
}
