...
async function sendSMS(recipientPhone, messageText) {
  try {
    const response = await fetch('https://sms.arkesel.com/api/v2/sms/send', {
      ...
      signal: AbortSignal.timeout(30000) // 30s timeout for Arkesel API
    });
    const data = await response.json();
...
async function main() {
  ...
  const progressFile = path.join(__dirname, 'kickoff_morning_progress.json');
  ...
  console.log('=============================================================');
  console.log('       SENA ACADEMY CAMPAIGN: SATURDAY MORNING SCHEDULE KICKOFF');
  console.log('=============================================================');
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    const smsText = `Sena Academy: Founding Builders kickoff is rescheduled to 7:00 PM GMT tonight (Saturday). Roster lock is at 6:00 PM today. This is your final chance to complete your registration at the GHS 100 early-bird discount before it reverts to GHS 200 at 6 PM. Secure seat: https://senaacademy.org/enroll`;

    const emailSubject = 'Schedule Update: Founding Builders Kickoff (7:00 PM Tonight)';
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff; color: #1e293b;">
        <h2 style="color: #0f172a; margin-top: 0; font-size: 20px; font-weight: 700; border-bottom: 1px solid #0f172a; padding-bottom: 15px;">🕒 Schedule Update: Live Kickoff at 7:00 PM Tonight</h2>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">Hi ${firstName},</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">Please note that our live kickoff learning session has been rescheduled to **7:00 PM GMT tonight** (Saturday, August 1st).</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">To accommodate final roster configurations and ensure everyone receives their portal credentials before kickoff, registration will remain open at the <strong>GHS 100</strong> early-bird rate until <strong>exactly 6:00 PM GMT today</strong>.</p>
        <p style="font-size: 15px; line-height: 1.6; color: #334155; margin: 20px 0;">After 6:00 PM today, the cohort roster will lock, and registration pricing will revert to the standard price of <strong>GHS 200</strong> permanently.</p>
        
        <div style="background-color: #f8fafc; border-left: 4px solid #0f172a; padding: 15px; margin: 25px 0; border-radius: 4px;">
          <h4 style="margin: 0 0 10px 0; color: #0f172a; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Updated Saturday Schedule:</h4>
          <ul style="margin: 0; padding-left: 20px; font-size: 13.5px; line-height: 1.6; color: #475569;">
            <li style="margin-bottom: 8px;"><strong>Roster Lock & Cut-off:</strong> 6:00 PM GMT today (pricing reverts to GHS 200).</li>
            <li style="margin-bottom: 8px;"><strong>Live Kickoff Class:</strong> 7:00 PM GMT tonight (Saturday, August 1st).</li>
            <li style="margin-bottom: 0;"><strong>Onboarding:</strong> Access codes and dashboard login details are sent immediately upon successful payment.</li>
          </ul>
        </div>

        <div style="text-align: center; margin: 30px 0;">
          <a href="https://senaacademy.org/enroll" style="background-color: #0f172a; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 14px; display: inline-block;">Complete Registration & Secure GHS 100 Seat</a>
        </div>
    ...
    console.log(`[${i + 1}/${pending.length}] Dispatching schedule update alert to ${contact.name}...`);
  ...
  console.log('\nSchedule update campaign complete.');
}
