...
export default function FacilitatorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/');
      } else if (profile && profile.role !== 'facilitator' && profile.role !== 'admin') {
        router.push('/student');
      }
    }
  }, [user, profile, loading, router]);

  if (loading || !user || !profile || (profile.role !== 'facilitator' && profile.role !== 'admin')) {
    return <LoadingScreen message="Gating Facilitator Area..." />;
  }

  const navLinks = [
    { name: 'Grading Dashboard', href: '/facilitator', icon: ClipboardList },
  ];
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-zinc-900 bg-zinc-950 p-6 justify-between shrink-0">
        <div className="space-y-8">
          <div className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-supporting-purple to-primary-blue flex items-center justify-center">
              <UserCheck className="h-4.5 w-4.5 text-white" />
            </div>
            <span className="text-sm font-bold tracking-tight text-white">SENA MENTOR</span>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                    active
                      ? 'bg-zinc-900 text-white border-l-2 border-l-supporting-purple shadow-inner'
                      : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/30'
                  }`}
                >
                  <Icon className={`h-4.5 w-4.5 ${active ? 'text-supporting-purple' : 'text-zinc-500'}`} />
                  {link.name}
                </Link>
              );
            })}
        ...
        <div className="space-y-4 pt-6 border-t border-zinc-900">
          <div className="flex items-center gap-3 px-2">
            <div className="h-8 w-8 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xs font-semibold text-zinc-305 text-supporting-purple">
            ...
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="h-7 w-7 rounded-lg bg-gradient-to-tr from-supporting-purple to-primary-blue flex items-center justify-center">
                    <UserCheck className="h-4 w-4 text-white" />
                  </div>
                  <span className="text-xs font-bold text-white uppercase tracking-wider">Facilitator Portal</span>
                </div>
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                        active
                          ? 'bg-zinc-900 text-white border-l-2 border-l-supporting-purple'
                          : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/30'
                      }`}
                    >
                      <Icon className={`h-4.5 w-4.5 ${active ? 'text-supporting-purple' : 'text-zinc-500'}`} />
        ...
        <header className="h-16 border-b border-zinc-900 flex items-center justify-between px-6 bg-zinc-950/80 backdrop-blur sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2">
              <span className="text-xs font-mono text-zinc-500">Grading Space</span>
