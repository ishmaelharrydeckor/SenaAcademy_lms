...
export default function FacilitatorLayout({ children }: { children: React.ReactNode }) {
  return (
    <ThemeProvider>
      <FacilitatorLayoutContent>{children}</FacilitatorLayoutContent>
    </ThemeProvider>
  );
}

function FacilitatorLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/');
      } else if (profile && profile.role !== 'facilitator') {
  ...
  if (loading || !user || !profile || profile.role !== 'facilitator') {
    return <LoadingScreen message="Verifying Mentor Access..." />;
  }
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                    active
                      ? 'bg-bg-surface-hover text-text-primary border-l-2 border-l-supporting-purple font-semibold shadow-inner'
                      : 'text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover/30'
                  }`}
                >
                  <Icon className={`h-4.5 w-4.5 ${active ? 'text-supporting-purple' : 'text-text-secondary'}`} />
                  {link.name}
                </Link>
              );
            })}
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${
                        active
                          ? 'bg-bg-surface-hover text-text-primary border-l-2 border-l-supporting-purple'
                          : 'text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover/30'
                      }`}
                    >
                      <Icon className={`h-4.5 w-4.5 ${active ? 'text-supporting-purple' : 'text-text-secondary'}`} />
        ...
        <header className="h-16 border-b border-border-brand flex items-center justify-between px-6 bg-bg-canvas/80 backdrop-blur sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2">
              <span className="text-xs font-mono text-text-secondary">Grading Space</span>
